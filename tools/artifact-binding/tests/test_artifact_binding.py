...
def test_the_committed_corpus_behaves_as_its_manifest_claims() -> None:
    import check_vectors

    check_vectors.FAILURES.clear()
    assert check_vectors.main() == 0
    assert check_vectors.FAILURES == []


def test_the_checker_reports_a_corpus_with_no_passing_member() -> None:
    import check_vectors

    check_vectors.FAILURES.clear()
    check_vectors.check_published_key([], b"\x00" * 32)
    assert any("no plain verified member" in line for line in check_vectors.FAILURES)
    check_vectors.FAILURES.clear()
...
def _corpus_copy(tmp_path: Path) -> Path:
    """A writable copy of the committed corpus, for mutating its claims."""
    import shutil
...
def _run_checker_against(target: Path, monkeypatch: pytest.MonkeyPatch) -> int:
    import check_vectors

    check_vectors.FAILURES.clear()
    monkeypatch.setattr(check_vectors, "HERE", target)
    return check_vectors.main()


def test_the_checker_refuses_a_manifest_whose_expectation_is_wrong(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import check_vectors

    target = _corpus_copy(tmp_path)
    path = target / "MANIFEST.json"
    payload = _load(path)
    for entry in payload["vectors"]:
        if entry["expected"]["verdict"] == "failed":
            entry["expected"]["verdict"] = "verified"
            break
    path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    assert _run_checker_against(target, monkeypatch) == 1
    assert any("reference verifier answered" in line for line in check_vectors.FAILURES)
    check_vectors.FAILURES.clear()


def test_the_checker_refuses_counts_that_disagree_with_the_entries(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import check_vectors
    ...
    path = target / "MANIFEST.json"
    payload = _load(path)
    payload["counts"]["verified"] = 99
    path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    assert _run_checker_against(target, monkeypatch) == 1
    assert any("declares counts" in line for line in check_vectors.FAILURES)
    check_vectors.FAILURES.clear()


def test_the_checker_refuses_a_missing_member_and_a_stale_digest(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import check_vectors
    ...
    path = target / "MANIFEST.json"
    payload = _load(path)
    payload["vectors"][0]["manifest"] = "cases/nowhere/binding/manifest.json"
    payload["corpusDigest"] = "0" * 64
    path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    assert _run_checker_against(target, monkeypatch) == 1
    assert any("is missing" in line for line in check_vectors.FAILURES)
    assert any("corpusDigest" in line for line in check_vectors.FAILURES)
    check_vectors.FAILURES.clear()


def test_the_checker_refuses_a_regrade_member_that_shares_no_archive(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import check_vectors

    target = _corpus_copy(tmp_path)
    regrade_manifest = target / "cases" / "regrade-changed-verifier" / "trial" / "binding"
    record = json.loads((regrade_manifest / "manifest.json").read_text(encoding="utf-8"))
    record["source_archive_digest"] = "0" * 64
    (regrade_manifest / "manifest.json").write_bytes(jcs.canonical_bytes(record))
    (regrade_manifest / "manifest.sig").write_text(
        sign.sign_bytes(
            bytes.fromhex(
                "9d61b19deffd5a60ba844af492ec2cc44449c5697b326919703bac031cae7f60"
            ),
            jcs.canonical_bytes(record),
        ).hex(),
        encoding="utf-8",
    )
    assert _run_checker_against(target, monkeypatch) == 1
    assert any("source_archive_digest" in line for line in check_vectors.FAILURES)
    check_vectors.FAILURES.clear()
