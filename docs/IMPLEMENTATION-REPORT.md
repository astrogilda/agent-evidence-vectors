...
| `ts-verify` | TypeScript | first-party consumer rail | its vendored set (118 vectors) + cross-rail parity tests | pass (see note 2) |
| `py-verify` | Python | first-party consumer rail (standalone) | its vendored set (118 vectors) + parity tests | pass (see note 2) |
| MCP server rail `_aee.py` | Python | first-party consumer rail | its vendored set (132 vectors) + parity tests | pass (see note 2) |
...
are covered by the two reference rails plus the two consumer stacks (TS rail, MCP rail)
...
2. **The consumer rails carry the round-7 rules but their vendored vector sets lag
   the reference corpus** (the TS rail 118, the MCP rail 132 vs the reference 138). The rules
   themselves are verified by cross-rail parity tests plus each rail's own suite
   and mutation checks; a full re-vendor of all 138 into the TS and MCP rails is tracked
