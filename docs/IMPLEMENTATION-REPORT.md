...
Corpus SSOT: vectors/MANIFEST.json (suiteRevision 27, 258 vectors: 60 accept, 196 reject, 2 indeterminate).
...
`vectors/MANIFEST.json`, suiteRevision 27: **258 vectors (60 accept, 196 reject, 2 indeterminate)**.
...
|---|---|---|---|---|
| Reference rail (`aee/`) | Go | spec author | reference corpus, suiteRevision 27 | **258 / 258** |
| Reference rail (`packaging/run_vectors.py`) | Python | spec author | reference corpus, suiteRevision 27 | **258 / 258** |
...
| `ts-verify` | TypeScript | spec author | its vendored set (258 vectors) + cross-rail parity tests | pass (see note 2) |
| `py-verify` | Python | spec author | its vendored set (258 vectors) + parity tests | pass (see note 2) |
| MCP server rail `_aee.py` | Python | spec author | its vendored set (258 vectors) + parity tests | pass (see note 2) |
...
2. **The consumer rails carry the suiteRevision-26 corpus.** The TypeScript rail,
   the standalone Python rail and the MCP server rail each vendor all 258 vectors of
   suiteRevision 26 byte-for-byte (`VENDOR-STAMP.json` pins the source spec digest,
   ...
   parity-tested on it, and replays the full 258. The three rails are two vendored
...
Nor does agreement on 258 vectors say anything about the surface no vector
