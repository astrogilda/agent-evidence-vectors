...
- **Spec.** A sentence was added to the `attackResults` paragraph (L920-932):
...
- **Spec.** The coverage paragraph (L907-912) now states the three sets are a
...
The coverage-partition membership rule (spec L912-914: the three sets are a
...
(L1302-1304), and a payload's fields "mean nothing until its signature verifies"
(L1724-1726). A record carrying no signature at all is therefore settled before the
...
- **Limits, not choices.** "The set of attacks actually executed therefore
  ...
  an evidencing heuristic in its place" (L928-940). Nothing in the carried bytes
...
- **Consumer policy outside the verdict this suite reads.** A consumer MAY admit
  ...
  substrate rows outright (L1159-1162), MAY bound a named key with a validity
  window (L1247), MAY coherence-check a row against the pinned posture
  (L1263-1267). None of these can move the verdict, because validity "is a function
  of carried bytes alone and holds identically for every consumer" (L1828-1831);
...
of those statements recompute to `pass_indirect` (L1146-1156).
...
boundary is incoherent" (L1263-1267). No verifier can decide whether an
...
fail-closed, and adds the append-only rule across minor versions (L847-855),
with the argument for closing the set written down rather than asserted
(L857-866). Three reject vectors (`bad-823`, `bad-824`, `bad-825`) and three
