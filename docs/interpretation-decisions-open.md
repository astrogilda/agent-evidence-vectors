...
- **Spec.** A sentence was added to the `attackResults` paragraph (L894-906):
...
- **Spec.** The coverage paragraph (L881-886) now states the three sets are a
...
The coverage-partition membership rule (spec L886-888: the three sets are a
...
(L1259-1261), and a payload's fields "mean nothing until its signature verifies"
(L1599-1601). A record carrying no signature at all is therefore settled before the
...
- **Limits, not choices.** "The set of attacks actually executed therefore
  ...
  an evidencing heuristic in its place" (L902-914). Nothing in the carried bytes
...
- **Consumer policy outside the verdict this suite reads.** A consumer MAY admit
  ...
  substrate rows outright (L1125-1128), MAY bound a named key with a validity
  window (L1204), MAY coherence-check a row against the pinned posture
  (L1220-1224). None of these can move the verdict, because validity "is a function
  of carried bytes alone and holds identically for every consumer" (L1703-1706);
...
of those statements recompute to `pass_indirect` (L1112-1122).
...
boundary is incoherent" (L1220-1224). No verifier can decide whether an
...
fail-closed, and adds the append-only rule across minor versions (L821-829),
with the argument for closing the set written down rather than asserted
(L831-840). Three reject vectors (`bad-823`, `bad-824`, `bad-825`) and three
